...
# def index_docs(documents):
    # if documents:
        ...
        # st.write(f"Indexed {len(documents)} documents successfully.")
    # else:
        # st.write("No documents to index.")
...
# if uploaded_file:
    ...
    # st.session_state.indexed_documents = None
    # st.session_state.indexing_complete = False  # Reset indexing flag to start the process again
...
# # Ensure the context persists across page loads
# if "history" not in st.session_state:
    # st.session_state.history = []

# if "indexed_documents" not in st.session_state or not st.session_state.indexed_documents:
    # st.error("No document data available. Please upload a file and try again.")
    # st.stop()

# # Display the context text box in the sidebar
# if st.session_state.get("indexing_complete", False):
    # # Use the template as default value if no context exists
    # if "context" not in st.session_state:
        # st.session_state.context = DEFAULT_TEMPLATE
    
    # # Display context text box in the sidebar
    # context = st.sidebar.text_area("Provide context for your question here", value=st.session_state.context, height=200)

    # # Save the context into session_state to retain when user switches pages
    ...
    # if question and context:
        ...
        # st.session_state.history.append((question, answer, file_path))

    # for q, a, src in st.session_state.history:
        # st.chat_message("user").write(q)
        # st.chat_message("assistant").write(f"{a}\n\n*Source: {os.path.basename(src)}*")
